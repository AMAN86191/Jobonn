...
const UpdateJobScreen = ({ navigation, route }: any) => {
  ...
  useEffect(() => {
    if (route.params?.job) {
      ...
      const rawJob = jobParam.rawJob || jobParam;
      console.log('Prefilling UpdateJobScreen with rawJob:', rawJob);

      // Map experience
      const expStr = rawJob.experience || '';
      let isFresher = true;
      let minExp = '';
      let maxExp = '';
      if (expStr && expStr.toLowerCase() !== 'fresher') {
        isFresher = false;
        // Parse range like "6-9 Years" or "6-9"
      ...
      // Map locations
      let locs: string[] = [];
      if (rawJob.locations && Array.isArray(rawJob.locations)) {
        locs = rawJob.locations;
      } else if (jobParam.location) {
        locs = jobParam.location.split(',').map((l: string) => l.trim()).filter(Boolean);
      }

      // Map skills
      let skillNames: string[] = [];
      if (rawJob.skills && Array.isArray(rawJob.skills)) {
        skillNames = rawJob.skills.map((s: any) => s.skill_name || s.name || '').filter(Boolean);
        // Pre-populate skillMap with IDs of existing skills
        setSkillMap(prev => {
          const next = { ...prev };
          rawJob.skills.forEach((s: any) => {
            const name = s.skill_name || s.name;
            const id = s.id;
            if (name && id) {
              next[name.toLowerCase().trim()] = id;
      ...
      } else if (jobParam.skills && Array.isArray(jobParam.skills)) {
        skillNames = jobParam.skills;
      }

      // Map benefits
      let benefitNames: string[] = [];
      if (rawJob.benefits) {
        benefitNames = rawJob.benefits.split(',').map((b: string) => b.trim()).filter(Boolean);
      } else if (jobParam.benefits && Array.isArray(jobParam.benefits)) {
        benefitNames = jobParam.benefits;
      }

      // Map languages
      let languagesList: string[] = [];
      if (rawJob.languages) {
        languagesList = rawJob.languages.split(',').map((l: string) => l.trim()).filter(Boolean);
      }

      // Map education
      let educationList: string[] = [];
      if (rawJob.education) {
        educationList = rawJob.education.split(',').map((e: string) => e.trim()).filter(Boolean);
      }

      // Map custom questions
      let questionsList: string[] = [];
      if (rawJob.additional_questions && Array.isArray(rawJob.additional_questions)) {
        questionsList = rawJob.additional_questions.map((q: any) => q.question).filter(Boolean);
      }

      // Map employmentType key back to label
      const getEmploymentTypeLabel = (typeKey: string) => {
        const type = (typeKey || '').toLowerCase();
        if (type.includes('full_time') || type === 'full-time' || type === 'fulltime') return 'Full-Time';
        if (type.includes('part_time') || type === 'part-time' || type === 'parttime') return 'Part-Time';
      ...
      if (rawJob.expiry_date) {
        try {
          deadline = new Date(rawJob.expiry_date);
      ...
      const prefilled: StepData = {
        industry: rawJob.industry?.industry_name || jobParam.category || '',
        title: rawJob.job_title?.job_name || jobParam.title || '',
        department: rawJob.department?.department_name || jobParam.department || '',
        location: locs,
        employmentType: getEmploymentTypeLabel(rawJob.job_type),
        minSalary: String(rawJob.salary_from || ''),
        maxSalary: String(rawJob.salary_to || ''),
        salaryType: rawJob.salary_type === 'monthly' ? 'monthly' : 'yearly',
        description: rawJob.job_description || '',
        category: rawJob.industry?.industry_name || jobParam.category || '',
        skills: skillNames,
        skillInput: '',
        isFresher,
        ...
        openings: String(rawJob.openings || '1'),
        benefits: benefitNames,
        benefitInput: '',
        applicationDeadline: deadline,
        urgent: !!rawJob.is_urgent,
  ...
  const handlePostJob = async () => {
    try {
      ...
      const isEditMode = !!route.params?.job;
      ...
      const payload: any = {
        ...
        status: isEditMode ? (route.params.job.rawJob?.status || route.params.job.status || "active").toLowerCase() : "active",
      ...
      if (isEditMode) {
        payload.job_id = route.params.job.id;
      }

      console.log('UpdateJob Payload:', payload);
      const response = await dispatch(EditPostJobSlice(payload)).unwrap();
      console.log('UpdateJob response:', response);

      if (response && (response.status === true || response.status_code === 200 || response.status_code === '200')) {
        Toast.show({
          type: 'success',
          text1: isEditMode ? 'Job Updated' : 'Job Published',
          text2: response.message || (isEditMode ? 'Job updated successfully!' : 'Job published successfully!'),
        });
        if (isEditMode) {
          const originalRawJob = route.params.job.rawJob || route.params.job;
          const jobRes = response.job || {};
          const mergedRawJob = {
            ...originalRawJob,
            ...jobRes,
            job_title: jobRes.job_title || (selectedJobTitleId === originalRawJob.job_title_id ? originalRawJob.job_title : { id: selectedJobTitleId, job_name: data.title }),
            company: jobRes.company || originalRawJob.company,
            department: jobRes.department || (selectedJobDepartmentId === originalRawJob.job_department_id ? originalRawJob.department : { id: selectedJobDepartmentId, department_name: data.department }),
            industry: jobRes.industry || (selectedJobIndustryId === originalRawJob.job_industry_id ? originalRawJob.industry : { id: selectedJobIndustryId, industry_name: data.industry }),
            locations: jobRes.locations || data.location,
            skills: jobRes.skills || data.skills,
            job_skills: jobRes.job_skills || originalRawJob.job_skills,
          };
          const state = navigation.getState();
          if (state && state.routes) {
            const routes = state.routes
              .filter((r: any) => r.name !== 'UpdateJobScreen')
              .map((r: any) => {
                if (r.name === 'ManagerJobDetails') {
                  return {
                    ...r,
                    params: { ...r.params, job: mergedRawJob },
                  };
                }
                return r;
              });
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: error?.message || error?.response?.data?.message || 'Failed to process job request',
      });
    } finally {
      setLoading(false);
    }
        ...
        <Animated.View style={styles.footer}>
          ...
          >
            <Text style={styles.nextBtnText}>
              {loading ? (route.params?.job ? 'Updating...' : 'Posting...') : (step === STEPS.length - 1 ? (route.params?.job ? 'Update Job' : 'Post Job') : 'Continue')}
            </Text>
            {loading ? (
