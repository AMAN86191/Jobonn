...
import { candidateNotifications } from '../../data/jobonnStaticData';

const NOTIFICATIONS = candidateNotifications;

const FILTER_TABS = ['All', 'Unread', 'Jobs', 'Interviews'];

const CandidateNotificationsScreen = ({ navigation }: any) => {
  const [activeTab, setActiveTab] = useState('All');
  const [notifications, setNotifications] = useState(NOTIFICATIONS);

  const filtered = notifications.filter(n => {
    if (activeTab === 'Unread') return !n.read;
    if (activeTab === 'Jobs') return n.type === 'job' || n.type === 'application' || n.type === 'shortlist';
...
const styles = StyleSheet.create({
  ...
  headerTitle: { ...Typography.body, color: Colors.textPrimary, fontWeight: '800' },
  unreadBadge: {
...
export default CandidateNotificationsScreen;
